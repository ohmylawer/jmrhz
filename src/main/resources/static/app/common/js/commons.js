...
function setformdata(data){
	for(key in data){
		if($("."+key).attr("type")=="checkbox"){
			for( var i=0;i<$("."+key+'[type="checkbox"]').length;i++ ){
				for( var j=0;j<data[key].split(',').length;j++ ){
					if( $("."+key+'[type="checkbox"]')[i].value == data[key].split(',')[j] ){
						$("."+key).eq(i).attr('checked', true);
					}
				}
			}
			$('#'+key).val(data[key])
...
function checkVal(name, value){
	var text = '<input type="hidden" name="'+name+'" id="'+name+'" class="form-control must" />';
	for( var i=0;i<value.length;i++ ){
		text += '<input type="checkbox" class="'+name+'" value="'+value[i].value+'" /> '+value[i].value+'&emsp;&emsp;'
	}
	$('.'+name+'s').html(text);
	$('.'+name).change(function(){
		var val = [];
		for( var i=0;i<$('.'+name+':checked').length;i++ ){
			val.push($('.'+name+':checked').eq(i).val())
		}
		$('#'+name).val(val)
	})
}
...
function initselect(id,arry){
	//$("#"+id).html("");
	var html = $("#"+id).html('<option value="">请选择</option>');
	$.each(arry,function(i){
...
function $ajax(obj){
	...
	var url = urlobj.urls ? _ip+urlobj.urls : urlobj.url;
	var async = obj.async;
	...
	var error = obj.error;
	var type = obj.type;
	var contentType = obj.contentType ? obj.contentType : 'application/x-www-form-urlencoded';
